...
@interface AudioPlayerController (){
    ...
    FMDetailModel *_playingModel; // 正在播放的model
...
- (void)updateAudioPlayer{
    ...
    playerItem = [AVPlayerItem playerItemWithURL:[NSURL URLWithString:[NSString stringWithFormat:@"http://mbod.qingting.fm/%@",[[model.mediainfo valueForKey:@"bitrates_url"] valueForKey:@"file_path"][0]]]];
...
- (void)updateUIDataWith:(FMDetailModel *)model{
    
    self.titleLabel.text = model.title;
    self.singerLabel.text = model.update_time;
...
#pragma mark - 后台UI设置
- (void)setLockViewWith:(FMDetailModel*)model currentTime:(CGFloat)currentTime
{
    NSMutableDictionary *musicInfo = [NSMutableDictionary dictionary];
    // 设置Singer
    [musicInfo setObject:model.update_time forKey:MPMediaItemPropertyArtist];
    // 设置歌曲名
